...
(function () {
  ...
  async function renderModelsTab(el, sb) {
    ...
    list.querySelectorAll(".npc-spawn-btn").forEach((b) => b.onclick = async () => {
      ...
      const mapId = getCurrentMapId();
      // NPC inicia sem rota — fica parado no ponto onde foi solto.
      // Criamos uma "rota" de 1 waypoint só para fixar a posição de spawn
      // (evita que o fallback do npc-tick herde uma rota aleatória do mapa).
      const { data: route } = await sb.from("npc_routes").insert({ name: "Parado " + Date.now(), map_id: mapId }).select().single();
      await sb.from("npc_waypoints").insert([
        { route_id: route.id, seq: 0, x: p.position.x, z: p.position.z, y: p.position.y, pause_ms: 999999 },
      ]);
      ...
      alert("NPC criado parado neste ponto. Para dar uma rota, vá na aba 'Rotas'.");
    });
